...
const sampleTranscript: TranscriptLine[] = [
  {
    ...
    text: {
      ...
      ko: '소라는 새벽빛 미래 도시로 안내합니다.',
      ar: 'سورا ترشدك عبر مدينة مستقبلية مضاءة بالفجر.',
  ...
  },
  {
    ...
    text: {
      ...
      ko: '네온과 새벽빛이 어우러지는 가운데 드론이 앞으로 미끄러지듯 움직입니다.',
      ar: 'تتحرك طائرة بدون طيار إلى الأمام بينما تمتزج أضواء النيون مع شروق الشمس.',
  ...
  },
  {
    ...
    text: {
      ...
      ko: '주인공이 등장하고 은빛 망토가 바람에 흩날립니다.',
      ar: 'يظهر البطل وعباءته الفضية تتمايل في الريح.',
...
export default function SubtitleGeneratorPage({ locale }: { locale: Locale }) {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!selectedFile) {
      setFormError(isZh ? '请先选择要处理的文件。' : 'Please select a file first.');
      if (!hasPaid) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(body?.error ?? (isZh ? '生成字幕失败，请稍后重试' : 'Subtitle generation failed. Please retry later.'));
      }

      const data = (await response.json()) as { data?: TranscriptionResponse };
      if (!data.data) {
        throw new Error(isZh ? '未取得字幕结果，请稍后再试' : 'No subtitle result received. Please try again.');
      }
      ...
      setStatus('idle');
      setFormError(error instanceof Error ? error.message : isZh ? '生成字幕失败，请稍后重试' : 'Subtitle generation failed. Please retry later.');
    }
  };
  ...
  const handleCheckout = async () => {
    try {
      ...
      if (!response.ok) {
        const body = await response.json().catch(() => ({}));
        throw new Error(body?.error ?? (isZh ? '无法创建支付链接' : 'Unable to create checkout link.'));
      }

      const data = (await response.json()) as { data?: { url?: string } };
      const url = data?.data?.url;
      if (!url) {
        throw new Error(isZh ? '支付链接生成失败，请稍后重试' : 'Failed to create checkout URL. Please try again.');
      }
      window.location.href = url;
    } catch (error) {
      setPaymentError(error instanceof Error ? error.message : isZh ? '支付初始化失败' : 'Payment initialisation failed.');
    } finally {
  ...
  const languages = isZh ? languageOptionsZh : languageOptionsEn;
    ...
    if (result) {
      const langLabel =
        ...
        (result.language
          ...
          : { label: isZh ? '识别结果' : 'Transcribed result', value: 'default' });
  ...
  }, [result, selectedLanguages, provider, languages, isZh]);
    ...
    <>
      ...
      <header className="space-y-4 text-center">
        <h1 className="text-3xl font-semibold text-slate-50">
          {isZh ? 'Sora 视频字幕生成器' : 'Sora Subtitle Studio'}
        </h1>
      ...
      <section className="grid gap-8 rounded-3xl border border-primary/20 bg-[#0e132b]/80 p-8 shadow-glow lg:grid-cols-[1.1fr_0.9fr]">
          ...
          <div className="lg:col-span-2">
            {hasPaid && (
              <div className="mb-4 flex items-center gap-3 rounded-2xl border border-emerald-500/40 bg-emerald-500/10 p-4 text-sm text-emerald-200">
                ...
                <div>
                  <p className="font-semibold">{isZh ? '支付已完成' : 'Payment confirmed'}</p>
          ...
          <div className="rounded-2xl border border-primary/30 bg-[#0b1224]/80 p-6">
              ...
              <div className="flex items-center gap-3 text-slate-200">
                ...
                <div>
                  <h2 className="text-sm font-semibold uppercase tracking-[0.3em] text-primary">
                    {isZh ? '字幕引擎' : 'Subtitle engine'}
                  </h2>
                  <p className="text-xs text-slate-500">
                    {isZh ? '选择 Sora 视频并配置转写选项' : 'Upload your Sora file and configure transcription.'}
                  </p>
                </div>
              </div>
            ...
            <label className="mt-6 block text-sm font-medium text-slate-200">
              ...
              >
                ...
                <div className="space-y-1">
                  <p className="text-slate-200">{isZh ? '拖拽文件到此处，或点击选择文件' : 'Drag your file here or click to browse'}</p>
                  <p className="text-xs text-slate-500">{uploadHint}</p>
                  {selectedFile && (
                    <p className="text-xs text-slate-300">
                      {isZh ? '已选择：' : 'Selected:'} {selectedFile.name} ({(selectedFile.size / (1024 * 1024)).toFixed(2)} MB)
                    </p>
                  )}
                </div>
                <input
            ...
            <div className="mt-6 grid gap-4 sm:grid-cols-2">
              <label className="block text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">
                {isZh ? '转写服务' : 'Transcription engine'}
              ...
              <fieldset className="space-y-2 text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">
                <legend>{isZh ? '输出语言' : 'Output languages'}</legend>
            ...
            {status === 'uploading'
              ? isZh ? '生成中…' : 'Generating…'
              : hasPaid
                ? isZh ? '生成字幕文件' : 'Generate subtitles'
                : isZh
                  ? '先支付后生成字幕'
        ...
        <aside className="rounded-2xl border border-primary/20 bg-[#0b1224]/80 p-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-sm font-semibold uppercase tracking-[0.3em] text-primary">
                  {isZh ? '输出预览' : 'Subtitle preview'}
                </h3>
                <p className="text-xs text-slate-500">
                  {isZh ? '多语言字幕与下载链接将显示在这里' : 'Multilingual subtitles and download links will appear here.'}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="mt-6 rounded-2xl border border-dashed border-primary/20 bg-[#050816]/80 p-6 text-center text-sm text-slate-400">
            {status === 'idle' && (
              <p>{isZh ? '提交视频后，会在这里展示字幕片段与多语言下载链接。' : 'Submit your file to preview subtitle segments and download links.'}</p>
            )}
            {status === 'uploading' && (
        ...
        <div className="space-y-6 text-slate-200">
          <header className="space-y-2">
            <h2 className="text-xl font-semibold text-slate-50">
              {isZh ? '完成支付以生成字幕' : 'Complete payment to generate subtitles'}
            </h2>
          ...
          <div className="rounded-2xl border border-primary/20 bg-[#0b1224]/70 p-4 text-sm text-slate-300">
            <div className="flex items-center justify-between">
              <span className="font-semibold text-slate-100">{isZh ? '字幕生成服务' : 'Subtitle generation'}</span>
              <span className="text-lg font-semibold text-primary">$9.9</span>
            </div>
